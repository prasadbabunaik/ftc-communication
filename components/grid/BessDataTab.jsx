...
// On the footer total row (`total`), the multi-month split collapses to a
// single combined total (matching the print/Excel exports).
function CodMonthCell({ row, rangeMonths, total = false }) {
  ...
  if (total) {
    const sum = rangeMonths.reduce((s, ym) => s + (row.codRangeMonths?.[ym] ?? 0), 0);
    return sum ? <>{fmt(sum)}</> : <span className="text-slate-300">0</span>;
  }
  ...
  return (
    <tr className={`border-t ${grand ? 'border-slate-400 bg-slate-200 font-black' : 'border-slate-300 bg-slate-100 font-bold'}`}>
      ...
      <td className="px-3 py-2 text-center tabular-nums text-violet-800">
        <CodMonthCell row={{ codInRefMonth: totals.codInRefMonth, codRangeMonths: totals.codRangeMonths }} rangeMonths={rangeMonths} total />
      </td>

  ...
  return (
    <div className="rounded border border-border">
      <table className="w-full text-xs min-w-[900px]">
        <thead className="sticky top-[156px] lg:top-[166px] z-[8] bg-slate-100">
          <tr className="bg-slate-100 text-slate-700 text-[10px] border-b border-slate-200">
            <th className="px-2 py-2 text-center font-bold whitespace-nowrap w-6">#</th>
            <th className="px-3 py-2 text-center font-bold whitespace-nowrap">Project</th>
          ...
          {projects.map((p, i) => {
            ...
            return (
              <tr key={p.id ?? i} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                ...
                <td className="px-3 py-1.5 text-center font-medium text-slate-800 max-w-[220px]">
                ...
                <td className="px-2 py-1.5 text-center font-mono">{fmt(p.totalCapacityMw)}</td>
                <td className="px-2 py-1.5 text-center font-mono">{fmt(ph.capacityAppliedMw)}</td>
                <td className="px-2 py-1.5 text-center font-mono text-emerald-700">{fmt(ph.ftcCompletedMw)}</td>
                <td className="px-2 py-1.5 text-center text-slate-500">{fmtDate(ph.ftcCompletedDate)}</td>
                <td className="px-2 py-1.5 text-center font-mono text-amber-700">{fmt(ph.capacityUnderFtcMw)}</td>
                <td className="px-2 py-1.5 text-center font-mono text-emerald-700">{fmt(ph.tocIssuedMw)}</td>
                <td className="px-2 py-1.5 text-center text-slate-500">{fmtDate(ph.tocIssuedDate)}</td>
                <td className="px-2 py-1.5 text-center font-mono text-amber-700">{fmt(ph.capacityUnderTocMw)}</td>
                <td className="px-2 py-1.5 text-center font-mono text-emerald-700">{fmt(ph.codDeclaredMw)}</td>
                <td className="px-2 py-1.5 text-center text-slate-500">{fmtDate(ph.codDeclaredDate)}</td>
                <td className="px-2 py-1.5 text-center font-mono text-amber-700">{codPend > 0.01 ? fmt(codPend) : '—'}</td>
                <td className="px-2 py-1.5 text-center font-mono text-blue-700">{fmt(ph.expectedApr26Mw)}</td>
              </tr>
            );
          })}
        </tbody>
        <tfoot>
          <tr className="bg-slate-100 font-semibold border-t-2 border-slate-300">
            ...
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.totalCapacityMw??0),0))}</td>
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.capacityAppliedMw??0),0))}</td>
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.ftcCompletedMw??0),0))}</td>
            <td />
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.capacityUnderFtcMw??0),0))}</td>
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.tocIssuedMw??0),0))}</td>
            <td />
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.capacityUnderTocMw??0),0))}</td>
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.codDeclaredMw??0),0))}</td>
            <td />
            ...
            <td className="px-2 py-1.5 text-center text-xs font-mono">{fmt(projects.reduce((s,p)=>s+Number(p.phases?.[0]?.expectedApr26Mw??0),0))}</td>
          </tr>
        </tfoot>
      </table>
    </div>

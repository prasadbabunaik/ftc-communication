...
async function statusByProjectAsOf(asOf) {
  const notes = await prisma.projectNote.findMany({
    where: { field: 'Status' },
    select: { projectId: true, newValue: true, createdAt: true, effectiveDate: true },
    orderBy: { createdAt: 'asc' },
  });
  const map = new Map();
  for (const n of notes) {
    const eff = n.effectiveDate ?? n.createdAt;
    if (eff.getTime() <= asOf.getTime()) {
      map.set(n.projectId, n.newValue);
    }
  }
  return map;
}
...
async function computeMatricesAsOf(asOf) {
  ...
  // filter sees the right status for `asOf`. Projects with no Status note
  // logged before `asOf` keep their initial (PENDING) status.
  const projects = rawProjects.map(p => {
    if (!p.contd4) return p;
    const hist = statusOverride.get(p.id);
    if (!hist) return { ...p, contd4: { ...p.contd4, status: 'PENDING' } };

...
const phaseRowSchema = z
  ...
  // Expected (MW) ≤ remaining capacity — what's still awaiting COD on this
  // source (Applied − COD completed). You can't expect to commission more
  // than is left.
  .refine(
    (p) => {
      const expected = parseFloat(p.expectedApr26Mw || '0') || 0;
      if (expected <= 0) return true;
      const applied = parseFloat(p.capacityAppliedMw || '0') || 0;
      const cod     = (p.codEvents ?? []).reduce((s, e) => s + (parseFloat(e.mw) || 0), 0);
      return expected <= applied - cod + 0.01;
    },
    { message: 'Exceeded: Expected (MW) is more than the remaining capacity (Applied − COD completed)', path: ['expectedApr26Mw'] }
  );

...
const commissioningEventSchema = z.object({
  ...
  // MW and MWh are BOTH optional, but at least one must be present (enforced
  // below). For BESS an event may carry only the energy (MWh) with no MW, or
  // only the MW with no MWh — each quantum is independent. Non-BESS sources only
  // ever supply MW, so the "at least one" rule keeps MW effectively required
  // for them.
  mw:      optionalDecimalStr,
  // Energy (MWh) — BESS only; parallel to the MW quantum.
  mwh:     optionalDecimalStr,
  // Date of the MW quantum. Required only when an MW is entered; an MWh-only
  // event is dated by its MWh date instead.
  date:    z.string().optional(),
  // MWh date — BESS only; independent of the MW date.
  mwhDate: z.string().optional(),
  remarks: z.string().optional(),
}).superRefine((e, ctx) => {
  const has = (v) => v != null && String(v).trim() !== '';
  const hasMw = has(e.mw);
  const hasMwh = has(e.mwh);
  // An event must record at least one quantum.
  if (!hasMw && !hasMwh) {
    ctx.addIssue({ code: 'custom', message: 'Enter a MW or MWh value', path: ['mw'] });
    return;
  }
  // The MW quantum drives the pipeline milestone dates, so it needs its own date.
  if (hasMw && !has(e.date)) {
    ctx.addIssue({ code: 'custom', message: 'Date is required', path: ['date'] });
  }
  // An MWh-only event still needs a date to anchor the event — its MWh date (or
  // an MW date, if one was typed).
  if (!hasMw && hasMwh && !has(e.mwhDate) && !has(e.date)) {
    ctx.addIssue({ code: 'custom', message: 'Enter the MWh date', path: ['mwhDate'] });
  }
});

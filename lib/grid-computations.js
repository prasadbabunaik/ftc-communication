...
export function computePipelineMatrix(projects, asOf = null, options = {}) {
  ...
  // Date-gate for folded hybrid components (mirrors milestoneAsOf's cutoff):
  // a component's milestone MW counts only once its date has arrived; a value
  // with no date is treated as completed-without-timing and always counts.
  let cutoff = asOf;
  if (!cutoff) { cutoff = new Date(); cutoff.setUTCHours(23, 59, 59, 999); }
  const gate = (mw, date) => (!date ? n(mw) : (new Date(date) <= cutoff ? n(mw) : 0));
  ...
  for (const proj of inPipeline) {
    ...
    // ── Including-Hybrid path: distribute components into source buckets ──
    if (foldHybrid && proj.plantType.isHybrid && proj.hybridComponentsJson) {
      if (seenHybridName.has(proj.name)) continue;
      seenHybridName.add(proj.name);
      for (const c of (proj.hybridComponentsJson.components ?? [])) {
        const key = `${region}|${c.sourceType}`;
        if (!matrix[key]) matrix[key] = emptyPipelineRow(region, c.sourceType);
        const row = matrix[key];
        row.totalCapacityMw  += n(c.totalMw);
        row.contd4CapacityMw += n(c.contd4Mw);
        row.appliedMw        += n(c.appliedMw);
        row.ftcApprovedMw    += gate(c.ftcMw, c.ftcDate);
        row.tocIssuedMw      += gate(c.tocMw, c.tocDate);
        row.codCompletedMw   += gate(c.codMw, c.codDate);
        row.expectedMw       += n(c.expectedMw);
      }
      continue;
    }

    // ── Default path: bucket the whole project by its source ──
    // (In fold mode, a hybrid lacking component data still lands here as
    //  HYBRID — we can't split it without the segregation sheet.)
    ...
    for (const ph of proj.phases) {
      ...
      row.ftcApprovedMw  += milestoneAsOf(ph.ftcEvents, asOf, ph.ftcCompletedDate, ph.ftcCompletedMw);
      row.tocIssuedMw    += milestoneAsOf(ph.tocEvents, asOf, ph.tocIssuedDate,    ph.tocIssuedMw);
      row.codCompletedMw += milestoneAsOf(ph.codEvents, asOf, ph.codDeclaredDate,  ph.codDeclaredMw);
...
export function buildPipelineRows(matrix, primaryKey, secondaryKey, filters = {}) {
  ...
  const sourceValues = filters.sources?.length ? SOURCE_ORDER.filter((s) => filters.sources.includes(s)) : SOURCE_ORDER;
...
export function computeMilestoneActivity(projects, from = null, to = null, componentSources = []) {
  const inRange = (d) => {
    if (!d) return false;
  ...
  for (const proj of projects.filter(isInFtcPipeline)) {
    ...
    if (!matrix[key]) matrix[key] = {
      ...
      // For hybrid rows we additionally split each milestone by its component
      // source (the phase's sourceType), so the pivot cell can read e.g.
      // "180 – BESS / 211.4 – Solar / 40.4 – Wind" like the Source-wise sheet.
      components: isHybrid ? { ftc: {}, toc: {}, cod: {} } : null,
    };
    const row = matrix[key];
    const addComp = (bucket, comp, mw) => { if (row.components) row.components[bucket][comp] = (row.components[bucket][comp] ?? 0) + mw; };
    for (const ph of (proj.phases ?? [])) {

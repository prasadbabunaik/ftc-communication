...
export function computePipelineMatrix(projects, asOf = null, options = {}) {
  ...
  for (const proj of inPipeline) {
    ...
    row.contd4CapacityMw += proj.contd4?.capacityApr26Mw != null
      ? n(proj.contd4.capacityApr26Mw)
      : n(proj.totalCapacityMw);
...
export function buildPipelineRows(matrix, primaryKey, secondaryKey, filters = {}) {
  ...
  // Including-Hybrid view folds hybrids into their source rows, leaving the
  // HYBRID bucket empty — drop it from the axis so no all-zero Hybrid row shows.
  if (filters.excludeHybrid) sourceValues = sourceValues.filter((s) => s !== 'HYBRID');

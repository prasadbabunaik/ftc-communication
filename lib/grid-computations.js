...
export function computePipelineMatrix(projects, asOf = null, options = {}) {
  ...
  const figuresOf = (proj) => {
    ...
    for (const ph of proj.phases) {
      f.appliedMw      += n(ph.capacityAppliedMw);
      // Date-gated milestone totals — per-event sums when `asOf` is set, else the
      // cached totals (see milestoneAsOf).
      f.ftcApprovedMw  += milestoneAsOf(ph.ftcEvents, asOf, ph.ftcCompletedDate, ph.ftcCompletedMw);
      f.tocIssuedMw    += milestoneAsOf(ph.tocEvents, asOf, ph.tocIssuedDate,    ph.tocIssuedMw);
      f.codCompletedMw += milestoneAsOf(ph.codEvents, asOf, ph.codDeclaredDate,  ph.codDeclaredMw);
      f.expectedMw     += n(ph.expectedApr26Mw);
    }
    return f;
  };
  ...
  for (const proj of inPipeline) {
    ...
    // ── Including-Hybrid path: split the hybrid's OWN (phase-based) figures
    //    across its component sources, weighted by each component's capacity
    //    share. This keeps the per-source split from the segregation sheet while
    //    guaranteeing the folded pieces sum back to exactly the same totals the
    //    Excl view shows (the segregation sheet's own funnel values can drift
    //    from the live commissioning data, which used to break reconciliation). ──
    if (foldHybrid && proj.plantType.isHybrid && comps.length) {
    ...
    //    hybrid lacking component data lands here as HYBRID (residual) — we can't
    //    split it without the segregation sheet, but buildPipelineRows keeps that
    //    residual row so the Incl total still reconciles with Excl. ──

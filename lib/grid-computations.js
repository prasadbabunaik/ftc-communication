...
// Single source of truth for a project's CONTD-4 issued capacity (MW).
//
// A linked CONTD-4 record is REQUIRED: a project with no CONTD-4 application has
// ZERO capacity under CONTD-4 — it is NOT assumed to be the plant total. This is
// what the project modal means by "No CONTD-4 application linked", and every
// CONTD-4 (MW) column (dashboard pipeline, FTC tracker, exports, application
// table) reads through here so they can never disagree again.
//
// When a CONTD-4 record DOES exist but its issued capacity wasn't explicitly
// entered, it defaults to the plant total (CONTD-4 assumed issued in full).
// Returns null when there is no record — callers that display use fmt/'—';
// callers that aggregate coerce via n().
export function contd4CapacityOf(project) {
  if (!project?.contd4) return null;
  const cap = project.contd4.capacityApr26Mw;
  return cap != null ? Number(cap) : n(project.totalCapacityMw);
}
...
// A project is "Commissioned" once its declared COD capacity reaches the plant
// total (small epsilon for rounding), OR an operator manually marked it so —
// the SAME rule the FTC tracker uses for its status badge. Everything else is
// still "Under Process". Used by the pipeline's "Exclude Commissioned" filter.
export function isProjectCommissioned(project) {
  if (project?.manuallyCommissioned) return true;
  const total = n(project?.totalCapacityMw);
  if (total <= 0) return false;
  const cod = (project?.phases ?? []).reduce((s, ph) => s + n(ph.codDeclaredMw), 0);
  return cod >= total - 0.01;
}
...
export function computePipelineMatrix(projects, asOf = null, options = {}) {
  ...
  const figuresOf = (proj) => {
    const f = {
      ...
      // CONTD-4 issued capacity — 0 when no CONTD-4 record is linked (see
      // contd4CapacityOf); matches the FTC tracker + modal so they reconcile.
      contd4CapacityMw: n(contd4CapacityOf(proj)),
  ...
  for (const proj of inPipeline) {
    ...
    if (foldHybrid && proj.plantType.isHybrid) {
      ...
      // 1. PREFER the live phase-level split — the single source of truth. Each
      //    phase carries its own SourceType, so its funnel figures go straight to
      //    that source (no proration); the project-level Total/CONTD-4 capacity is
      //    prorated by per-source applied share (→ capacity columns → equal split).
      //    This keeps the dashboard, hybrid breakdown and exports reconciled with
      //    the headline KPIs, instead of the frozen segregation snapshot. (Every
      //    seeded hybrid now also carries phases, so JSON below is a pure
      //    phaseless fallback.)
      if (phases.length) {
        const bySource = {};
        ...
        const contd4Cap = n(contd4CapacityOf(proj));
        sources.forEach((s, i) => {
          const key = `${region}|${s}`;
      ...
      // 2. Segregation JSON — phaseless seeded hybrids only. Split the project's
      //    phase-based figures by each component's capacity share; dedup the
      //    duplicate shadow rows the seed inserts (same name + JSON).
      if (comps.length) {
...
export function buildPipelineRows(matrix, primaryKey, secondaryKey, filters = {}) {
  ...
  // per-region breakdown) before the grand total. When scoped to a single
  // region (an RLDC), that consolidated breakdown IS that region's data, so it
  // is labelled with the region code (e.g. 'SR') rather than 'All India'.
  const consolidatedRegion = singleRegionRegionWise ? primaryValues[0] : 'All India';
  const aggBySecondary = {};
  for (const sv of secondaryValues) {
    const baseRegion = primaryKey === 'region' ? consolidatedRegion : sv;
...
// All-India per-source totals with hybrids FOLDED IN — i.e. WIND = pure wind +
// the wind component of every hybrid, etc. Drives the "Total <Source> including
// Hybrid" rows in the region-wise exports. Returns { WIND: {…funnel}, … }.
export function inclHybridSourceTotals(projects, asOf = null) {
  const m = computePipelineMatrix(projects ?? [], asOf, { foldHybridComponents: true });
  const out = {};
  for (const [key, r] of Object.entries(m)) {
    const src = key.split('|')[1];
    if (src === 'HYBRID') continue; // residual un-foldable hybrids stay separate
    if (!out[src]) out[src] = emptyPipelineRow('All India', src);
    for (const f of PIPELINE_NUMERIC_FIELDS) out[src][f] += r[f];
  }
  return out;
}

// Expand a region-wise pipeline table (from buildPipelineRows(matrix,'region',…))
// so each HYBRID leaf row is REPLACED by its constituent-source sub-rows
// (Wind/Solar/BESS/PSP), and "Total <Source> including Hybrid" rows are appended
// before the grand total. CONTD-4 is plant-level, so it is carried once on the
// first component of each hybrid group (rendered as a merged cell) rather than
// split. `hybridBreakup` = computeHybridComponentBreakup; `inclTotals` =
// inclHybridSourceTotals (pass null to skip the including-hybrid rows).
const INCL_HYBRID_SOURCES = ['WIND', 'SOLAR', 'BESS', 'PSP'];
export function expandRegionRowsWithHybrid(rows, hybridBreakup = {}, inclTotals = null) {
  const out = [];
  for (const r of rows) {
    // Append "including hybrid" rows immediately before the grand total.
    if (r.isTotal && inclTotals) {
      for (const src of INCL_HYBRID_SOURCES) {
        const t = inclTotals[src];
        if (t && (t.totalCapacityMw > 0.001 || t.appliedMw > 0.001)) {
          out.push({ ...t, region: 'All India', source: src, isInclHybridTotal: true });
        }
      }
    }

    const isHybridLeaf = r.source === 'HYBRID' && !r.isSubtotal && !r.isTotal;
    if (!isHybridLeaf) { out.push(r); continue; }

    const regionKey = r.isAllIndiaBreakdown ? 'All India' : r.region;
    const comps = (hybridBreakup[regionKey] ?? []).filter((c) => c.totalCapacityMw > 0.001 || c.appliedMw > 0.001);
    if (!comps.length) { out.push(r); continue; }
    comps.forEach((c, i) => {
      out.push({
        ...c,
        region: r.region,
        source: 'HYBRID',
        isAllIndiaBreakdown: r.isAllIndiaBreakdown,
        isHybridComponent: true,
        component: c.source,           // WIND / SOLAR / BESS / PSP
        hybridGroupSize: comps.length,
        hybridGroupFirst: i === 0,
        // CONTD-4 shown once for the whole hybrid group (plant-level).
        contd4CapacityMw: i === 0 ? r.contd4CapacityMw : null,
      });
    });
  }
  return out;
...
// Compute the per-component Hybrid Breakdown for EVERY pipeline hybrid, using
// the same attribution ladder as the FTC-pipeline fold (first available wins):
//
//   1. Commissioning PHASES — the live data the FTC tracker edits. Each phase
//      carries a concrete SourceType, so its funnel figures go straight to its
//      component row; the project-level Total/CONTD-4 capacity is prorated by
//      applied share (fallback: capacity columns, then equal). Changes made in
//      the tracker therefore reflect here immediately.
//   2. Segregation JSON — legacy fallback for seeded hybrids that have no
//      phases yet (the Excel "Source wise Segregation" sheet). Shadow duplicate
//      rows share the same JSON, so those are counted once per project name.
//   3. Per-source capacity columns — totals only, for hybrids with neither.
//
// `asOf` honours the same convention as milestoneAsOf() — a milestone counts
// only once its date has arrived (date ≤ cutoff; cutoff = asOf or today's
// end-of-day). componentSources (optional): restrict to the listed component
...
export function computeHybridBreakdown(projects, asOf = null, componentSources = []) {
  ...
  const rowOf = (region, hybridType, sourceType) => {
    const key = `${region}|${hybridType}|${sourceType}`;
    if (!matrix[key]) {
      matrix[key] = {
        region, hybridType, sourceType,
        // contd4Mw starts null: CONTD-4 is issued at PLANT level (Contd4Phase
        // has no sourceType), so a per-source figure only exists where the
        // seeded segregation sheet recorded one — never fabricated by
        // proration. null renders as "—".
        totalMw: 0, contd4Mw: null, appliedMw: 0,
  ...
  const CAP_COL = { WIND: 'windCapacityMw', SOLAR: 'solarCapacityMw', BESS: 'bessCapacityMw' };
  // JSON shadow duplicates share the same hybridComponentsJson — count once.
  // (No dedup on the phase path: each row's phases are its own data.)
  const seenJsonName = new Set();

  for (const proj of hybrids) {
    const region = proj.region.code;
    const phases = proj.phases ?? [];

    // ── 1. Phase-level split (live tracker data) ──
    if (phases.length) {
      const hybridType = proj.plantType.label;
      const bySource = {};
      for (const ph of phases) {
        if (componentSources.length && !componentSources.includes(ph.sourceType)) continue;
      ...
      const sources = Object.keys(bySource);
      if (!sources.length) continue;   // all phases filtered out by componentSources
      let weights = sources.map((s) => bySource[s].applied);
      if (!weights.some((w) => w > 0)) weights = sources.map((s) => n(proj[CAP_COL[s]]));
      if (!weights.some((w) => w > 0)) weights = sources.map(() => 1);
      const wBase = weights.reduce((a, b) => a + b, 0);
      const totalCap  = n(proj.totalCapacityMw);
      // No contd4Mw here — CONTD-4 is plant-level; there is no real per-source
      // split to show (see rowOf).
      sources.forEach((s, i) => {
        const row = rowOf(region, hybridType, s);
        row.totalMw    += totalCap  * (weights[i] / wBase);
        row.appliedMw  += bySource[s].applied;
        row.ftcMw      += bySource[s].ftc;
        row.tocMw      += bySource[s].toc;
        row.codMw      += bySource[s].cod;
        row.expectedMw += bySource[s].exp;
      });
      continue;
    }

    // ── 2. Segregation JSON (legacy, phaseless) ──
    const data = proj.hybridComponentsJson;
    if ((data?.components ?? []).length) {
      if (seenJsonName.has(proj.name)) continue;
      seenJsonName.add(proj.name);
      ...
      for (const c of data.components) {
        ...
        const row = rowOf(region, hybridType, c.sourceType);
        row.totalMw    += n(c.totalMw);
        // The sheet's per-component CONTD-4 is the only REAL per-source value.
        row.contd4Mw    = (row.contd4Mw ?? 0) + n(c.contd4Mw);
    ...
    if (capSplit.length) {
      const base = capSplit.reduce((s, [, v]) => s + v, 0);
      const totalCap = n(proj.totalCapacityMw);
      // No contd4Mw — CONTD-4 is plant-level (see rowOf).
      for (const [s, v] of capSplit) {
        if (componentSources.length && !componentSources.includes(s)) continue;
        const row = rowOf(region, proj.plantType.label, s);
        row.totalMw += totalCap * (v / base);
      }
    }
  }
...
export function aggregateProjectForExport(proj, asOf = null) {
  ...
  return {
    ...
    totalCapacityMw: n(proj.totalCapacityMw),
    // 0 when no CONTD-4 record is linked (see contd4CapacityOf); consistent with
    // the pipeline matrix, FTC tracker and modal.
    contd4CapacityMw: n(contd4CapacityOf(proj)),

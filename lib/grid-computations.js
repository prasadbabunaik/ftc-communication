...
// date.
//
// Live view ("now or later"): always return the cached aggregate MW. This
// matches the source Excel / Google-Sheet behaviour, which sums the
// "FTC Completed Capacity" column (col 9) without any date-gating — even
// rows whose FTC date is in the future (e.g. Tehri's FTC date 03-Jun-2026)
// or simply blank still count toward the current pipeline totals.
//
// Historical view ("as of a past date"): strict date-gating — sum only the
// per-event rows whose eventDate ≤ asOf, falling back to an all-or-nothing
// gate on the single legacy completion date when no events exist. A phase
// with cached MW but no temporal info at all (no events, no date) is
// considered "completed without recorded timing" and contributes its full
// cached MW; returning 0 there would silently hide capacity reported by
// the operations team via the aggregate cell.
function milestoneAsOf(events, asOf, fallbackDate, fallbackMw) {
  if (!asOf) return n(fallbackMw);
  // Today's end-of-day (UTC). If asOf is at or after this point, the user
  // is asking for the current pipeline state — match the source workbook
  // and short-circuit to the cached aggregate.
  const todayEod = new Date(); todayEod.setUTCHours(23, 59, 59, 999);
  if (asOf >= todayEod) return n(fallbackMw);
  // Past asOf — date-gated point-in-time replay.
  ...
  if (fallbackDate) {
    return new Date(fallbackDate) <= asOf ? n(fallbackMw) : 0;
  }
  return n(fallbackMw);
}
...
//
// `asOf` honours the same convention as milestoneAsOf() — for live-view
// (asOf >= today) we sum the cached MW directly; only historical replay
// date-gates the per-component MWs against their stored milestone dates.
export function computeHybridBreakdown(projects, asOf = null) {
  const cleared = projects.filter(p => p.contd4?.status === 'CLEARED' && p.hybridComponentsJson);
  const matrix  = {};

  const todayEod = new Date(); todayEod.setUTCHours(23, 59, 59, 999);
  const liveView = !asOf || asOf >= todayEod;
  const gate = (mw, date) => {
    if (liveView) return n(mw);
    if (!date) return n(mw);
    return new Date(date) <= asOf ? n(mw) : 0;
  };

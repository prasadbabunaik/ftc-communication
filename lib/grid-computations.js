...
export function computeMilestoneActivity(projects, from = null, to = null) {
  ...
  for (const proj of projects.filter(isInFtcPipeline)) {
    ...
    const source = getProjectSource(proj);
    const key    = `${region}|${source}`;
    if (!matrix[key]) matrix[key] = { region, source, ftc: 0, toc: 0, cod: 0, ftcCount: 0, tocCount: 0, codCount: 0 };
    const row = matrix[key];
    for (const ph of (proj.phases ?? [])) {
      for (const e of (ph.ftcEvents ?? [])) if (inRange(e.eventDate)) { const mw = n(e.capacityMw); row.ftc += mw; row.ftcCount++; totals.ftc += mw; totals.ftcCount++; }
      for (const e of (ph.tocEvents ?? [])) if (inRange(e.eventDate)) { const mw = n(e.capacityMw); row.toc += mw; row.tocCount++; totals.toc += mw; totals.tocCount++; }
      for (const e of (ph.codEvents ?? [])) if (inRange(e.eventDate)) { const mw = n(e.capacityMw); row.cod += mw; row.codCount++; totals.cod += mw; totals.codCount++; }
    }
  }

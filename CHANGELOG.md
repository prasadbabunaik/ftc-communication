...
### Added
- **Including / Excluding Hybrid bifurcation** on the **FTC Pipeline**, **Source-wise** and **FTC/TOC/COD Activity** tabs — a toggle (next to the Source filter on those tabs) that switches how hybrid projects are counted. **Excluding** (default, unchanged): hybrids sit in their own `HYBRID` row. **Including**: each hybrid's per-component capacity is folded into its source row, so e.g. the Wind row = pure-wind projects + the wind component of every hybrid — and the now-empty `HYBRID` row is dropped entirely. Implemented via a `foldHybridComponents` option on `computePipelineMatrix` / `computeMilestoneActivity` and an `excludeHybrid` axis option on `buildPipelineRows` ([lib/grid-computations.js](lib/grid-computations.js)), driven by a `?hybrid=incl` URL param. The six top stat cards keep the default (Excluding) basis. Pipeline/Source-wise component figures come from the segregation sheet and need not sum to a hybrid's aggregate phase totals; Activity folds the per-date events (so its totals are conserved). See [components/grid/SummaryPageClient.jsx](components/grid/SummaryPageClient.jsx) (`HybridModeToggle`).

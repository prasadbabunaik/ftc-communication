...
### Added
- **Including / Excluding Hybrid bifurcation on the FTC Pipeline tab** — a toggle (next to the Source filter, pipeline tab only) that switches how hybrid projects are counted. **Excluding** (default, unchanged): hybrids sit in their own `HYBRID` row. **Including**: each hybrid's per-component capacity (from `hybridComponentsJson`) is folded into its source row, so e.g. the Wind row = pure-wind projects + the wind component of every hybrid (and there's no separate Hybrid row). Implemented via a `foldHybridComponents` option on `computePipelineMatrix` ([lib/grid-computations.js](lib/grid-computations.js)) and a `?hybrid=incl` URL param; scoped to the FTC Pipeline tab only (stat cards, Source-wise and other tabs keep the default view). Component figures come from the segregation sheet and need not sum to the hybrid's aggregate phase totals. See [components/grid/SummaryPageClient.jsx](components/grid/SummaryPageClient.jsx) (`HybridModeToggle`).
